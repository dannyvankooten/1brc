...
static inline const char* parse_number(int* dest, const char* s)
{
    ...
    // Check if the character is '-' using bitwise operations
    int isMinus = !(*s ^ '-'); // isMinus is 1 if *s is '-', else 0

    mod -= isMinus << 1; // if isMinus is 1, then 1 << 1 is 2, mod becomes -1
    s += isMinus; // Move the pointer if the first character is '-'
    len += isMinus; // Increment length if the first character is '-'

    // Check if the second character is '.' using bitwise operations
    int isDot = !(s[1] ^ '.');

    // Calculate both possible *dest values
    int destDot = ((s[0] * 10) + s[2] - ('0' * 11)) * mod;
    int destNotDot = (s[0] * 100 + s[1] * 10 + s[3] - ('0' * 111)) * mod;

    // Conditionally set *dest
    *dest = destDot * isDot + destNotDot * !isDot;

    // Conditionally set len
    s += (4 * isDot) + (5 * !isDot);
